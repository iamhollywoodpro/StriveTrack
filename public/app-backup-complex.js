...
async function showDashboard() {
    ...
    syncUserPointsFromServer().catch(error => {
        console.log('Could not sync from server:', error.message);
    });
    updatePointsDisplay();
...
async function loadDashboardData() {
    await Promise.all([
        ...
        loadHabits(), // ENABLED: Load habits on dashboard
        loadMedia(),
        loadAchievements(),
        loadDailyChallenges(),
        loadAdminData(),
...
function createHabitElement(habit, showWeekView = true) {
    return createSimpleHabitCard(habit);
}

// FIXED: Simple habit card that actually works
function createSimpleHabitCard(habit) {
    ...
    div.innerHTML = `
        <div class="flex justify-between items-start mb-4">
            <div>
                <h3 class="text-white font-semibold text-lg mb-1">${habit.name}</h3>
                ${habit.description ? `<p class="text-white/60 text-sm">${habit.description}</p>` : ''}
            </div>
            <div class="px-3 py-1 rounded-full text-xs font-medium ${completedToday ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}">
        ...
        <div class="flex justify-between text-sm text-white/70 mb-4">
            ...
            <span>🎯 ${weeklyTarget}/week</span>
        </div>
        
        <div class="flex gap-2">
            <button onclick="toggleHabitCompletion('${habit.id}')" 
                    class="flex-1 py-2 px-4 rounded-lg font-medium transition-all ${completedToday ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-green-500 hover:bg-green-600'} text-white"
                    ${completedToday ? 'title="Mark as incomplete"' : 'title="Mark as complete"'}>
...
// Sample habits creation
function createSampleHabits() {
    const sampleHabits = [
        { id: 'habit_1', name: 'Morning Workout', description: '30 minutes of exercise', category: 'fitness', weekly_target: 5, created_at: new Date().toISOString() },
        { id: 'habit_2', name: 'Drink Water', description: '8 glasses throughout the day', category: 'health', weekly_target: 7, created_at: new Date().toISOString() },
        { id: 'habit_3', name: 'Read Daily', description: 'Read for 20 minutes', category: 'learning', weekly_target: 6, created_at: new Date().toISOString() }
    ];
